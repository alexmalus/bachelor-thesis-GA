...
class StrengthChecker extends HtmlElement {
    public $default_text='verificare rezistenta parolei';
    function init(){
        parent::init();
    ...
    function setResponse($p, $out){
        ...
        if(!$p){
            ...
            $this->js(null,$j)->text('Parola este ok! :)')->execute();
        }
    }
    ...
    function showStrengLevel($response){
        ...
        if($response=='E mult prea scurta')return $this->js(null,$j)->addClass('low');
	...
	function checkByCrackLib($password){
		...
		if(file_exists($cl) && is_executable($cl)){
			...
			if(strlen($password)<4)return "E mult prea scurta!";
			if(strlen($password)<6)return "Inca este scurta!";
			return "OK";
		}
	}
}
