...
class Auth_Basic extends AbstractController {
     ...
     * that data or $auth->model (preferred)   $auth->get('numeUser') will always point to the login field
    ...
    /** This function verifies numeUser and password. Password must be supplied in plain text. Does not affect currently 
     * logged in user */
     ...
     */
    function loggedIn($user=null,$pass=null){ //$numeUser,$password,$memorize=false){
    ...
    function memorizeModel(){
        ...
        $this->info['numeUser']=$this->info[$this->login_field];
    ...
    function createForm($page){
        ...
        $email=$email?$email->caption:'Mail-ul Dumneavoastra';

        $password=$this->model->hasField($this->password_field);
        $password=$password?$password->caption:'Parola';

        $form->addField('Line','numeUser',$email);
        $form->addField('Password','parola',$password);
    ...
    function showLoginForm(){
        ...
        $f=$this->form;
        if($f->isSubmitted()){
            if($this->verifyCredentials($f->get('numeUser'), $f->get('parola'))){             
                $this->login($f->get('numeUser'));
                $this->loggedIn($f->get('numeUser'),$f->get('parola'));
                exit;
            }
            $f->getElement('parola')->displayFieldError('Informatii introduse gresite');
        }
        return $p;
    }
    /** Do not override this function. */
